...
# Extend to 24 hours
claude-session-restore list --max-age-hours 24

# Load full context
claude-session-restore load "/path/to/session-id.jsonl"
```
...
The binary will be at `target/release/claude-session-restore` (or `claude-session-restore.exe` on Windows).

### Install to PATH

#### Linux/macOS
```bash
cargo install --path claude-session-restore
```

#### Windows (Git Bash/MSYS2)
```bash
cargo build --release
cp target/release/claude-session-restore.exe ~/.local/bin/
```
...
├── claude-session-restore/    # CLI tool
│   ├── Cargo.toml
│   └── src/main.rs
